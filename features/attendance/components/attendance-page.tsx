...
import { ATTENDANCE_STATUS, type AttendanceStatus } from '../schemas/attendance.schema';
...
export function AttendancePage() {
    ...
    // When attendance data loads, sync it to local state
    const initializeStatusMap = useCallback((data: typeof attendanceData) => {
        if (!data) return;
    ...
    // Fetch students in selected class (from attendance data or fallback from students query)
    const handleClassOrDateChange = () => {
        setStatusMap({});
        if (attendanceData) initializeStatusMap(attendanceData);
    };

    // Toggle a student's status through the cycle: PRESENT → ABSENT → LEAVE → PRESENT
...
// Small inline hook to get students by class (avoids another api file)
